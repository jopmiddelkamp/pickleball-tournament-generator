...
describe("buildPublicView", () => {
  ...
  it("lists everyone signed up by name, gender and level, in arrival order, confirmed before waiting", () => {
    ...
    expect(view.signedUp[0]).toEqual({ id: "p1", name: "Player p1", gender: "F", level: 3, confirmed: true, position: null });
    const last = view.signedUp[view.signedUp.length - 1]!;
    expect(last).toEqual({ id: `p${cap + 2}`, name: `Player p${cap + 2}`, gender: "F", level: 3, confirmed: false, position: 2 });
  });
