...
const valid = { location: "Balanca", maxCourts: 4, playersPerCourt: 5, rounds: 6, gameTarget: 11, roundMinutes: null, minLevel: null, algorithmId: "greedy" };
...
describe("parseEventDefaults", () => {
  ...
  it("round-trips a minimum level", () => {
    expect(parseEventDefaults(JSON.stringify({ ...valid, minLevel: 3 }))).toEqual({ ...valid, minLevel: 3 });
  });
  ...
  it.each([
    ...
    ["minimum level out of range", JSON.stringify({ ...valid, minLevel: 7 })],
    ["a cookie from before minimum levels existed", JSON.stringify((({ minLevel: _omit, ...older }) => older)(valid))],
