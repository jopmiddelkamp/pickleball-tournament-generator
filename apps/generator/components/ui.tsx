...
/** A medal for a podium place: a disc carrying the place, coloured by the `--medal` token the class sets. */
export function Medal({ place }: { place: 1 | 2 | 3 }) {
  return (
    <span className={`medal medal--${place}`} aria-label={String(place)}>
      {place}
    </span>
  );
}
